            ...
            <form action="<?php echo Config::get('URL'); ?>user/changeUserRole_action" method="post">
                <div class="form-group">
                    <?php if (Session::get('user_account_type') == 1) { ?>
                        <input type="submit" class="form-control" name="user_account_upgrade"
                               value="Upgrade my account (to Premium User)"/>
                    <?php } else if (Session::get('user_account_type') == 2) { ?>
                        <input type="submit" class="form-control" name="user_account_downgrade"
                               value="Downgrade my account (to Basic User)"/>
                    <?php } ?>
                </div>
            </form>
        </div>
